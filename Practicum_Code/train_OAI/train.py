...
SNAPSHOTS_KNEE_GRADING = os.path.abspath("/scratch/o/oespinga/shiyuxin/snapshots_knee_grading_OAI"
    # os.path.join(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset',  default="C:/University/2022_Fall/CHL5207/Knee_Osteoarthritis/"
                                              "OAI_raw/OAI_cropped_manual/OAI_jpg")
    parser.add_argument('--matchbook', default="C:/University/2022_Fall/CHL5207/Knee_Osteoarthritis/OAI_raw/"
                                               "OAI_cropped_manual/KL_match_book.csv")
    parser.add_argument('--brightcontrast', default=True)
    ...
    cur_snapshot = time.strftime('%Y_%m_%d_%H_%M_%S') + args.param_test
    ...
    train_img = os.listdir(os.path.join(args.dataset, "train"))
    train_KL = [imgdict.get(key) for key in os.listdir(os.path.join(args.dataset, "train"))]
    ...
    for kl in range(5):
        ...
        train_cats_length.append(num_img_KL)

    oversample_size = int(sum(train_cats_length) / 5)
    train_files = []
    print("Oversample Size: "+ str(oversample_size))
    np.random.seed(args.seed)
    for kl in range(5):
        # files = np.array(os.listdir(
        #     os.path.join(args.dataset, "train", str(kl))))
        files = np.array(train_img)[np.where(np.array(train_KL)==str(kl))]
    ...
    if os.path.isfile(os.path.join(args.snapshots, 'mean_std.npy')):
        ...
        train_ds = KneeGradingDataset(args.dataset,
                                      ...
                                      augment=CenterCrop(220),
                                      stage='train',
                                      kl_matchbook=args.matchbook)
        # print(train_ds[0][0])
